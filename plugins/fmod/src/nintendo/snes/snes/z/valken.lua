...
function user.init()
	...
	function spc700_comm(offset, data)
		...
		if offset == 0x1B2141 then
			...
			if se == 0x20 and text_se_mute == 1 then return 0x00 end
			MES("se:%02X", se)
			if xvib:play(se) == 1 then return end
		end
		return data
	end
	...
	function valken_text(offset)
		...
		if a_reg == 0x0000 then
			local y_reg = cpu.state["X"].value
			--local mem = cpu.spaces["program"]
			local dump_bytes = {}
			for i = 0, 5 do
				local val = mem:read_u8(0x7E0000 + 0x2030 + i)
				table.insert(dump_bytes, string.format("%02X", val))
			end
			local hex_string = table.concat(dump_bytes, "")
			print(string.format("%04X|%s", y_reg, hex_string))
		end
	end

	function text_addr_check(offset, data)
		local pc_reg = cpu.state["PC"].value
		local s_reg = cpu.state["S"].value
		local a_reg = cpu.state["A"].value
		local x_reg = cpu.state["X"].value
		local y_reg = cpu.state["Y"].value
		MES("d:%02X PC:%06X S:%04X A:%04X X:%04X Y:%04X", data, pc_reg, s_reg, a_reg, x_reg, y_reg)
		if data == 0x02 and a_reg == 0xa702 then
			fmod:play(a_reg)
		end
		if data == 0x68 and a_reg == 0xa768 then
			fmod:play(a_reg)
		end
		if data == 0xce and a_reg == 0xa6ce then
			fmod:play(a_reg)
		end
		if data == 0xe8 and a_reg == 0xa6e8 then
			fmod:play(a_reg)
		end
		if data == 0x40 and a_reg == 0xa740 then
			fmod:play(a_reg)
		end

	end

	local CHAR_VOICE_MAP = {
		[0x1] = { 3,  1.20,  0.00, 1.10, 1.80 }, -- Jake (Zundamon Normal)
		[0x2] = { 2,  1.25,  0.05, 1.00, 1.70 }, -- Claire (Shikoku Metan Normal)
		[0x3] = { 7,  1.15, -0.05, 0.90, 2.00 }, -- Captain (Namine Ritsumu / Deep voice style)
		[0x4] = { 11, 1.10, -0.08, 0.85, 2.00 }, -- Herman (Kyomachi Seika / Older style)
		[0x5] = { 9,  1.20, -0.02, 1.00, 1.80 }, -- Kurt (Namine Ritsumu Normal)
		[0x6] = { 11, 1.20, -0.04, 0.90, 2.00 }, -- Veldark (Kurono Takehiro Normal)
		[0x7] = { 13, 1.10,  0.00, 1.00, 1.60 }, -- Enemy Soldier (Aoyama Ryusei Normal)
		[0x8] = { 12, 1.28,  0.02, 0.95, 1.80 }, -- Rick (Shirakami Kotarou Normal)
		[0x9] = { 14, 1.15,  0.00, 1.00, 1.70 }, -- Friendly Soldier 1 (Meisa Normal)
		[0xA] = { 22, 1.05, -0.06, 0.85, 2.00 }, -- President Shellmark (Zundamon Sawayaka)
		[0xB] = { 1,  1.20,  0.03, 1.05, 1.60 }, -- Friendly Soldier 2 (Shikoku Metan Normal)
	}
...
--		[0xa4c] = 0x9, -- President
	...
	local DialogueManager = {
		...
		-- 敵 (0x6)
		[(0x858E75 << 24) | 0xF04E] = 0x3961, -- 「全員退避だ。作戦通り、この基地は地球に落とす」
		[(0x858E75 << 24) | 0x2F6D] = 0x3962, -- 「グランビアの出撃準備を急げ。私が出撃する。」
		-- ジェイク (0x1)
		...
		-- 敵 (0x6)
		[(0x858F8E << 24) | 0x0311] = 0x4161, -- 「このグランビアはきさまにはとめられんぞ！」
		-- ジェイク (0x1)
		[(0x858F8E << 24) | 0x39F0] = 0x4111, -- 「要塞を地球に落とすなんて・・・ きさま、」
		[(0x858F8E << 24) | 0xED7E] = 0x4112, -- 「民間人をまきぞえにするつもりかっ！」
		-- 敵 (0x6)
		[(0x858F8E << 24) | 0x7CF0] = 0x4162, -- 「核を使わないだけましというものだ。いくぞっ！」
		...
		-- 敵 (0x6)
		[(0x858FED << 24) | 0x1665] = 0x4361, -- 「ばかな！たった一機のアサルトスーツに・・・」
		...
		-- カーツ (0x5)
		[(0x8593B2 << 24) | 0x4140] = 0x5851, -- 「了解！」※↑と同じセリフなので区別が出来ない
		...
		-- クレア (0x2)
		[(0x859787 << 24) | 0xF00B] = 0x6621, -- 「シャトルのサイロまではまだ遠いわ。気をつけて」

		-- Stage 5 / Scene 7 (サイロ最下層)
		-- ジェイク (0x1)
		[(0x8597AF << 24) | 0x66F1] = 0x6711, -- 「迷子になりそうだ」
		-- クレア (0x2)
		[(0x8597AF << 24) | 0x4AFD] = 0x6721, -- 「最下層は底なしです！ 落ちたらあがってこれないわ！」
		...
		-- ジェイク (0x1)
		[(0x859AFC << 24) | 0x0D66] = 0x7913, -- 「バーシスが危ない！カーツ、外にでるぞ！」

		-- Stage 6 / Scene A (機動兵器と対空砲)
		-- カーツ (0x5)
		[(0x859C34 << 24) | 0x6338] = 0x7A51, -- 「機動兵器だ！！」
		-- ジェイク (0x1)
		[(0x859C34 << 24) | 0x7AF1] = 0x7A11, -- 「対空砲が先だ！そんな奴に構うな！」
		-- カーツ (0x5)
		...
		-- ジェイク (0x1)
		[(0x859F4C << 24) | 0x5D2D] = 0x8A13, -- 「戦争の原因なんて問題じゃない！ この戦争で死んだ多くの人々への責任をとるんだ」
		-- 大統領 (0x9)
		[(0x859F4C << 24) | 0xD301] = 0x8A95, -- 「今の私にできることはこれぐらいだ・・」
		-- ジェイク (0x1)
		[(0x859FFA << 24) | 0x1101] = 0x8A14, -- 「こんな。ことで・・・」
		[(0x859FFA << 24) | 0x1F11] = 0x8A15, -- 「こんなことで責任をとったつもりなのかっ！」
	...
	function DialogueManager.get_id(a_val, i_val, c_val)
		...
		if ret == 0x5D81 then cid = 0 end
		if ret == 0x7431 then cid = 0 end
		if ret == 0x7A52 then cid = 0 end
		if ret == 0x7B31 then cid = 0 end
		if ret == 0x8D82 then cid = 0 end
		if ret == 0x8D83 then cid = 0 end
		if ret == 0x9211 then cid = 0 end
		if cid > 0 then ret = (ret&0xff0f) + (cid<<4) end
		return ret
	end
	...
	[0xf010] = "うわっ！",
	[0xf020] = "ジェイク、防御フィールド出力低下。危険です！",

	-- ==========================================
	-- STAGE 1
	-- ==========================================
	-- Stage 1 / Scene 1 (オープニング突入)
	[0x1131] = "これより本艦は、敵のコロニーへ突入する。",
	[0x1132] = "目標は、コロニー内で建造中の戦艦だ。",
	[0x1133] = "アサルトスーツ小隊は突入後発進しろ。",

	-- Stage 1 / Scene 2 (ジェイク発進)
	[0x1211] = "こちらジェイク、発進する！",

	-- Stage 1 / Scene 3 (粒子砲の警告)
	[0x1321] = "艦長！敵戦艦の艦首にエネルギー反応！",
	[0x1322] = "本艦の真正面です！",
	[0x1331] = "なにっ！もう粒子砲を撃てるというのか！",
	[0x1311] = "エネルギーユニットは、接続されちまったのか？",
	[0x1323] = "まだです！ でも時間がないわ！",
	[0x1312] = "了解。それは俺が叩く！ バーシスは後退してくれ。",

	-- Stage 1 / Scene 4 (ユニット位置確認)
	[0x1421] = "エネルギーユニットは敵艦艦尾にあるはずです。",
	[0x1422] = "レーダーで確認できるわ。",
	[0x1411] = "おーけー。でもまだ範囲外なようだ。",

	-- Stage 1 / Scene 5 (建造中への愚痴)
	[0x1541] = "くそっ！こいつのどこが建造中なんだよ！",
	[0x1511] = "ぼやくなよハーマン。",

	-- Stage 1 / Scene 6 (カーツの報告)
	[0x1651] = "こちらカーツです。",
	[0x1652] = "エネルギーユニットを確認しました！",
	[0x1653] = "距離約１３０！",
	[0x1611] = "了解！ まだまだ遠いな・・・",
	...
	[0x1741] = "うわぁ！",
	[0x1711] = "どうした？ハーマン！",
	[0x1742] = "脚に一発くらっちまった。でもまだやれるぜ！",
	[0x1712] = "ムリするな。バーシスに帰還しろ。",
	[0x1743] = "りょ、了解・・・",

	-- Stage 1 / Scene 8 (ユニット発見)
	[0x1811] = "こちらジェイク、",
	[0x1812] = "エネルギーユニットを発見した。",
	[0x1821] = "了解！ただちに破壊してください。",
	[0x1822] = "いそいでっ！",
	...
	-- ==========================================
	-- Stage 2 / Scene 1 (全機発進)
	[0x2131] = "全機発進だ！岩にぶつかるなよ。",
	[0x2121] = "侵入経路は追尾航法装置に入力してありますが、",
	[0x2122] = "こまかい岩は目視で避けてください。",
	[0x2111] = "どうせ、敵の迎撃があるんだ。うまくやるさ。",

	-- Stage 2 / Scene 2 (ミサイル接近)
	[0x2221] = "ミサイル多数接近！かわすより破壊して！",
	[0x2222] = "気をつけて！",

	-- Stage 2 / Scene 3 (中心部到達)
	[0x2311] = "中心部に到達した。ブースターを切り離す。",
	[0x2321] = "了解、慎重に進んでください。",

	-- Stage 2 / Scene 4 (近接機雷)
	[0x2421] = "岩盤に近接機雷の反応があります。",
	[0x2411] = "分かった。警戒する。",

	-- Stage 2 / Scene 5 (加工施設への疑問)
	[0x2511] = "加工施設にしちゃ敵が多すぎるな・・・。",

	-- Stage 2 / Scene 6 (ハーマン施設破壊)
	[0x2641] = "こちらハーマン、主な鉱石加工施設は破壊した。",
	[0x2621] = "了解、敵の攻撃に気を付けて帰還して下さい。",

	-- Stage 2 / Scene 7 (カーツ任務完了)
	[0x2751] = "こちらカーツ、こちらも任務はほぼ完了。",
	[0x2721] = "了解、あとはあなただけねジェイク。",
	[0x2711] = "わかっているさ。",

	-- Stage 2 / Scene 8 (巨大エネルギー反応)
	[0x2811] = "よし、これでここは壊滅したも同然だな。",
	[0x2812] = "ん？・・・なんだ？",
	[0x2821] = "どうかしましたか？",
	[0x2813] = "前方の岩盤にエネルギー反応だ。かなりでかい。",
	[0x2822] = "こちらでも捕らえました。いやな予感がするわ。",
	[0x2814] = "調べてみる。",
	...
	-- Stage 2 / Scene A (機動兵器破壊)
	[0x2A11] = "こちらジェイク、敵機動兵器を破壊した。",
	[0x2A21] = "おつかれさま、作戦終了です。帰還してください。",

	-- ==========================================
	-- STAGE 3
	-- ==========================================
	-- Stage 3 / Scene 1 (発進用意)
	[0x3131] = "全機発進用意！作戦通りアークノバ内に進行し",
	[0x3132] = "内部から基地を破壊せよ。",
	[0x3121] = "バーシスでぎりぎりまで接近しますが",
	[0x3122] = "敵の対空砲かは強力です。充分、注意してください。",

	-- Stage 3 / Scene 2 (進行ポイント指示)
	[0x3221] = "ハーマンはポイント１４、カーツはポイント２２を通過してアークノバに取りついて下さい。",
	[0x3241] = "了解！派手に決めるぜ！",

	-- Stage 3 / Scene 3 (侵入口探索)
	[0x3321] = "ジェイクはアークノバ下部の侵入口を探して下さい。",
	[0x3311] = "ああ、そのつもりだ。",

	-- Stage 3 / Scene 4 (ジェイクとりつき)
	[0x3411] = "こちらジェイク　アークノバにとりついた。",
	[0x3441] = "ちっ！先をこされちまったな。",

	-- Stage 3 / Scene 5 (カーツとりつき)
	[0x3551] = "カーツです。アークノバにとりつきました。",
	[0x3511] = "ようし、侵入口を見つけしだい突入しろ。",

	-- Stage 3 / Scene 6 (ハッチ破壊)
	[0x3611] = "アークノバのハッチを破壊した。",
	[0x3612] = "これより突入する。",
	[0x3631] = "よし、派手に暴れてやれ。",
	...
	-- Stage 3 / Scene 8 (指令室の位置)
	[0x3811] = "こちらジェイク、指令室はどこだ！？",
	[0x3821] = "あなたの真下にあります。左からまわりこんで！",

	-- Stage 3 / Scene 9 (地球落下計画)
	[0x3961] = "全員退避だ。作戦通り、この基地は地球に落とす。",
	[0x3962] = "グランビアの出撃準備を急げ。私が出撃する。",
	[0x3911] = "なに？基地を落とすだと、どういう事だ！",

	-- Stage 3 / Scene A (目標変更)
	[0x3A31] = "みんな、よく聞け！ 敵はアークノバを地球に落とすためのエンジンを作動させてしまった。攻撃目標をアーク・ノバのメイン・エンジンに変更する。",
	[0x3A21] = "繰り返します。全機、アークノバの上部にあるエンジンを狙ってください。",

	-- Stage 3 / Scene B (爆発確認)
	[0x3B21] = "アークノバ内部で大規模な爆発を確認！ 彼らはほんとうに要塞を地球に落とすつもりだわ。",

	-- Stage 3 / Scene C (格納庫突入)
	[0x3C11] = "敵の格納庫を発見！これより突入する。",
	[0x3C21] = "気をつけてください。",
	[0x3C22] = "内部には大型兵器の反応があります。",

	-- Stage 4 / Scene 1 (グランビア遭遇)
	[0x4161] = "このグランビアはきさまにはとめられんぞ！",
	[0x4111] = "要塞を地球に落とすなんて・・・ きさま、",
	[0x4112] = "民間人をまきぞえにするつもりかっ！",
	[0x4162] = "核を使わないだけましというものだ。いくぞっ！",

	-- ==========================================
	-- STAGE 3b
	-- ==========================================
	-- Stage 3b / Scene 2 (残り2分)
	[0x4221] = "アークノバ落下までの時間はあと２分です。エンジンに攻撃を集中してください。",

	-- Stage 3b / Scene 3 (グランビア撃破)
	[0x4361] = "ばかな！たった一機のアサルトスーツに・・・",

	-- Stage 3b / Scene 4 (残り1分)
	[0x4421] = "あと１分！！まだ間に合うわ！ おねがい！ジェイク！",

	-- Stage 3b / Scene 5 (エンジン破壊催促)
	[0x4521] = "はやくエンジンを破壊してください。 時間がありません。",

	-- Stage 3b / Scene 6 (作戦成功)
	[0x4621] = "ふぅ、間に合いました。 アークノバは地球への軌道から外れていきます。",
	[0x4611] = "了解・帰投する。",

	-- Stage 3b / Scene 7 (作戦失敗)
	[0x4721] = "もうアークノバの落下は止められません！ アークノバから離脱してください。危険です。",
	[0x4711] = "残念だ。こんなにも巨大な質量が地球に落ちるというのか・・・",
	...
	-- ==========================================
	-- Stage 4 / Scene 1 (追撃命令)
	[0x5161] = "大気圏突入ぎりぎりまで、やつらを追いつめろ！",
	[0x5171] = "了解！",
	[0x5181] = "りょ、了解！",

	-- Stage 4 / Scene 2 (大気圏突入の危機)
	[0x5221] = "艦長！ジェイクが",
	[0x5222] = "大気圏に突入してしまいます！",
	[0x5231] = "バーシスを降下させろ！地球におりてもかまわん。",

	-- Stage 4 / Scene 3 (収容要請と拒否)
	[0x5321] = "ジェイク、収容します。",
	[0x5322] = "できるだけバーシスに接近してください。",
	[0x5311] = "いや、まだだ！こいつらを落とす！",

	-- Stage 4 / Scene 4 (逃げ遅れ)
	[0x5461] = "そろそろ限界だな・・・よし、艇に戻るぞ！",
	[0x5481] = "ベルダーク少佐！",
	[0x5482] = "バ、バーニアの出力が上がらない・・・",
	[0x5483] = "た、たすけて・・・",
	[0x5462] = "いま行くぞ！リック！",
	[0x5471] = "むりです少佐、もう間に合いません！",
	[0x5463] = "くそっ・・・！",

	-- Stage 4 / Scene 5 (リック救出)
	[0x5511] = "敵の一機が逃げおくれたらしい。",
	[0x5512] = "助けてやってくれ！",
	[0x5521] = "むちゃです！",
	--[0x5522] = "バーシスのバランスが崩れてしまうわ！",
	[0x5522] = "バーシスのバランスが崩れてバラバーシスしてまうわ！",
	[0x5531] = "見殺しにはできん。やれるだけやってみろ！",
	[0x5523] = "わ、わかりました！",

	-- Stage 4 / Scene 6 (突入速度)
	[0x5621] = "突入角度が深すぎます！降下速度が落ちません！",

	-- Stage 4 / Scene 7 (安否確認)
	[0x5711] = "大丈夫か！クレア！",
	[0x5721] = "え、ええ・・・なんとか。あなたは？",
	[0x5712] = "こっちも大丈夫だ。",

	-- Stage 4 / Scene 8 (敵輸送艇接近)
	[0x5821] = "あっ！敵の輸送艇が接近してきます。",
	[0x5831] = "数は？",
	[0x5822] = "一隻だけのようですが、アサルトスーツを数機",
	[0x5823] = "射出しています。",
	[0x5832] = "ハーマン！カーツ！迎撃しろ！",
	[0x5841] = "了解！",
	[0x5851] = "了解！",

	-- Stage 4 / Scene 9 (損傷機体)
	[0x5911] = "俺もでるぞ！",
	[0x5921] = "だめです！あなたの機体は損傷がはげしいわ！",
	[0x5912] = "心配してもらうほどじゃないさ。",

	-- Stage 4 / Scene A (ハーマン撃墜)
	[0x5A41] = "くそっ！こいつ素早い！",
	[0x5A21] = "どうしました！？",
	[0x5A42] = "敵のリーダーらしい！こっちに向かって・・・",
	[0x5A43] = "ぐわぁっ！・・・・",
	--[0x5A44] = "（通信途絶のノイズ）",
	[0x5A11] = "どうした！？ハーマン！",

	-- Stage 4 / Scene B (反応消失)
	[0x5B21] = "ハーマンの機体の反応が消えました！",
	[0x5B11] = "ばかなっ！？誰がハーマンを！",

	-- Stage 4 / Scene C (因縁の通信割込)
	[0x5C11] = "こいつ、さっきの！",
	[0x5C61] = "また会ったな。",
	[0x5C12] = "なにっ！通信に割込んできた！？",
	[0x5C62] = "リックを助けてくれた礼はいわせてもらう。",
	[0x5C63] = "だが彼は返してもらうぞ。",
	[0x5C13] = "させるかっ！",
	...
	-- Stage 4 / Scene F (撤退と敬礼)
	[0x5F21] = "敵アサルトスーツ隊、撤退してゆきます・・・",
	[0x5F11] = "ちくしょう、ハーマン ・・・・ちくしょう！！",
	[0x5F31] = "総員、ハーマンに敬礼！",
	...
	-- ==========================================
	-- Stage 5 / Scene 1 (傍受と激励)
	[0x6121] = "艦長！敵基地からの通信を傍受しました！",
	[0x6122] = "シャトル発射の秒読みが開始されたようです！",
	[0x6131] = "AS隊へのブースターの装着は完了しているか？",
	[0x6123] = "はい！",
	[0x6132] = "よし、少し早いが射出しろ。",
	[0x6124] = "了解。",
	[0x6125] = "聞こえましたか？",
	[0x6111] = "ああ、・・・",
	[0x6126] = "ねえ、元気をだして。",
	[0x6127] = "ハーマンの事はあなたのせいじゃないわ。",
	[0x6112] = "わかってるよ、クレア。",

	-- Stage 5 / Scene 2 (戦闘機の警告)
	[0x6221] = "対空砲は無視していいわ。でも戦闘機には注意して。",

	-- Stage 5 / Scene 3 (ブースターパージ)
	[0x6311] = "ブースターの燃料がきれた！切り離す！",

	-- Stage 5 / Scene 4 (シャトル防衛線)
	[0x6471] = "これ以上すすませるな！シャトルをまもるんだ！",
	...
	-- Stage 5 / Scene 6 (発射場侵入)
	[0x6611] = "発射場に侵入する。",
	[0x6621] = "シャトルのサイロまではまだ遠いわ。気をつけて。",

	-- Stage 5 / Scene 7 (サイロ最下層)
	[0x6711] = "迷子になりそうだ。",
	[0x6721] = "最下層は底なしです！ 落ちたらあがってこれないわ！",
	...
	-- Stage 5 / Scene A (シャトル発射・ブースター要請)
	[0x6A11] = "しまった！まニアわなかったか！",
	[0x6A21] = "ロケットブースターを射出します。",
	[0x6A22] = "装着後、シャトルを追撃してください。",
	[0x6A12] = "わかった、早くしてくれ。",

	-- Stage 5 / Scene B (大空の迎撃)
	[0x6B71] = "相手は一機だけだ。撃ち落とせ！ シャトルをやらせるな！",

	-- Stage 5 / Scene C (シャトル破壊成功)
	[0x6C11] = "シャトルのロケットが機能を停止した。 こいつはもうスクラップだ！",

	-- Stage 5 / Scene D (帰還)
	[0x6D21] = "作戦成功です。バーシスに着艦してください。",
	[0x6D11] = "クレア？",
	[0x6D22] = "なに？",
	[0x6D12] = "この戦争は早く終わらせなきゃな・・・。 ハーマンのためにも。",
	[0x6D23] = "そうね、ジェイク。",

	-- Stage 5 / Scene E (引き離し/失敗用？)
	[0x6E11] = "シャトルのスピードが落ちない！ ひきはなされる！",

	-- Stage 5 / Scene F (大気圏離脱/ゲームオーバー用？)
	[0x6F21] = "シャトルがまもなく大気圏を離脱します・・・",
	[0x6F11] = "・・・すまん。",
	[0x6F22] = "ジェイク・・・",
	...
	-- ==========================================
	-- Stage 6 / Scene 1 (作戦会議と先行)
	[0x7131] = "さけて移動することはできんということか？",
	[0x7121] = "はい。あと６時間以内に合流地点に着かなければ",
	[0x7122] = "ソルジャーソウル作戦に参加できません。",
	[0x7123] = "そのためにはこの山を越えないと・・・",
	[0x7111] = "俺とカーツが先行して高射砲を破壊する。",
	[0x7132] = "陣地の近くには偵察機がうろついているはずだ。",
	[0x7133] = "できるだけ発見されないように行動しろ。",
	[0x7112] = "了解。いくぞカーツ。",
	[0x7151] = "はい。",
	...
	-- Stage 6 / Scene 3 (スノークス発見報告)
	[0x7371] = "こちらスノークス！敵のアサルトスーツだ！ あの艦の奴にちがいない。警戒せよ！",

	-- Stage 6 / Scene 4 (スカウト被発見)
	[0x7411] = "こちらジェイク！スカウトに見つかった！",
	[0x7431] = "バーシスの速度を落とすわけにはいかん！ 敵基地内に強行侵入しろ。",
	[0x7412] = "了解！",
	...
	-- Stage 6 / Scene 7 (山頂到達の報告)
	[0x7711] = "よし、やったぞ！",
	[0x7721] = "こちらバーシス！本艦はまもなく山頂に達します。",
	[0x7712] = "やばいな！急ぐぞ、カーツ！",

	-- Stage 6 / Scene 8 (建設機械)
	[0x7811] = "ここは本当に対空陣地なのか？ 建設機械ばかりじゃないか！",
	[0x7851] = "さあね、とにかくもう少し探索してみましょう。",

	-- Stage 6 / Scene 9 (高射砲移動)
	[0x7911] = "あった！あれだ。バーシス！高射砲を発見",
	[0x7912] = "したぞ！",
	[0x7951] = "大変です！高射砲が外にでて行きます！",
	[0x7913] = "バーシスが危ない！カーツ、外にでるぞ！",

	-- Stage 6 / Scene A (機動兵器と対空砲)
	[0x7A51] = "機動兵器だ！！",
	[0x7A11] = "対空砲が先だ！そんな奴に構うな！",
	[0x7A52] = "そうもいかない様です。",
	...
	-- Stage 6 / Scene D (格納庫被弾)
	[0x7D21] = "アサルトスーツ格納庫に被弾！",
	[0x7D11] = "待ってろよ！いまこいつをぶちこわしてやるからな。",
	...
	-- ==========================================
	-- Stage 7 / Scene 1 (バーシス浮上不能・決意)
	[0x8121] = "もうバーシスは浮上不能です！",
	[0x8122] = "あなたたちだけが頼りよジェイク",
	[0x8111] = "ああ、わかってるさ！",
	[0x8123] = "生きてかえってきて",
	[0x8112] = "ああ、かならず！",

	-- Stage 7 / Scene 2 (艦長の命令)
	[0x8231] = "生きている砲台は、全て開け、応戦するんだ！",
	[0x8221] = "了解！",

	-- Stage 7 / Scene 3 (数の脅威)
	[0x8311] = "敵はザコばかりだが数は多いぞ！注意しろ！",
	[0x8351] = "了解！ジェイクもムチャしないでください",
	...
	[0x8412] = "どうしたっ！？",
	[0x8423] = "バーシスが被弾！",
	[0x8424] = "でも、まだ大丈夫。もちこたえてるわ。",

	-- Stage 7 / Scene 5 (議事堂侵入と通信途絶)
	[0x8511] = "こちらジェイク！これより侵入する！",
	[0x8521] = "了解！議会の議員達を連行し・・・・・・・",
	[0x8512] = "！？・・・・",
	[0x8513] = "どうした！？バーシス！応答しろ、クレア！",

	-- Stage 7 / Scene 6 (孤独な突入)
	[0x8611] = "侵入口をみつけた。応答してくれバーシス！",
	[0x8612] = "だめか・・・",

	-- Stage 7 / Scene 7 (大統領連行と通信割込)
	[0x8711] = "この議会場は合衆国軍が押えた。投降しろ！",
	[0x8791] = "くっ！",
	[0x8712] = "きさまがシェルマーク大統領か！",
	[0x8792] = "・・・そうだ。",
	[0x8713] = "おまえを連行する！",
	[0x8761] = "そうはさせん！",
	[0x8714] = "きさまは！",
	...
	-- Stage 7 / Scene 8 (ベルダークの納得)
	[0x8811] = "俺を殺しても、もうどうにもならんぞ！",
	[0x8861] = "そんなことは関係ない！ 俺は俺自身が納得できるまで戦う！",

	-- Stage 7 / Scene 9 (大統領逃走)
	[0x8911] = "ばかなやつめ・・・はっ！大統領は？・・・",
	[0x8912] = "上かっ！",

	-- Stage 7 / Scene A (大統領の最期)
	[0x8A11] = "もうあきらめるんだな。",
	[0x8A91] = "そのつもりだ・・・",
	[0x8A12] = "きさまには責任をとってもらう。",
	[0x8A92] = "責任？なんの責任だ？",
	[0x8A93] = "この戦争は私が起こしたわけじゃない。",
	[0x8A94] = "時の流れが我々を戦争へと導いたにすぎん！",
	[0x8A13] = "戦争の原因なんて問題じゃない！ この戦争で死んだ多くの人々への責任をとるんだ。",
	[0x8A95] = "今の私にできることはこれぐらいだ・・",
	[0x8A14] = "こんな。ことで・・・",
	[0x8A15] = "こんなことで責任をとったつもりなのかっ！",

	-- Stage 7 / Scene B (デューク中尉からの通信)
	[0x8BA1] = "だれか！だれか応答してくれ！",
	[0x8B11] = "だれだっ！",
	[0x8BA2] = "第３２AS隊のデューク中尉だ！",
	[0x8BA3] = "敵の巨大な機動兵器が始動している！",
	[0x8BA4] = "増援を・・・",
	[0x8B12] = "そこはどこだ！？",
	[0x8BA5] = "国民議会ビルの地下の工場施設に・・・・・",
	[0x8BA6] = "うわあっ！",
	[0x8B13] = "応答しろ！デューク中尉！",
	[0x8B14] = "この戦争に終わりはないのか！？",

	-- Stage 7 / Scene C (兵士の生き方)
	[0x8C11] = "なぜだ！？なぜきさまら向かってくるんだ！？もう戦争は終わったんだぞ！！",
	[0x8C61] = "それは俺たちが兵士だからだ！ 生きているかぎり戦いを続ける、それが兵士の生きかたというものだ！",
	[0x8C12] = "だまれ！",

	-- Stage 7 / Scene D (リックの立ち塞がり)
	[0x8D81] = "おまえをベルダーク少佐の元にはいかせない！",
	[0x8D11] = "なんだと！",
	[0x8D82] = "少佐のじゃまをする奴は許さない。 少佐だけがこの国を救うことができるんだ！",
	[0x8D12] = "奴はそんな男じゃない！わからないのか！？ 奴は全てを破壊しようとしているんだぞ！",
	[0x8D83] = "それ以上言うなっ！！",
	...
	-- Stage 8 / Scene 1 (最終機動兵器ビルドヴォーグ)
	[0x9111] = "こ、この機動兵器は・・",
	[0x9161] = "ふっふっふ・・・・・・はーっはっはっは！",
	[0x9162] = "驚いたか！これが我が軍の最終機動兵器",
	[0x9163] = "ビルドヴォーグだ！こいつの前では貴様など",
	[0x9164] = "虫ケラにしかすぎん！",
	[0x9112] = "ばかな！",
	[0x9113] = "こんなものを持ちだして、どうしようというんだ！",
	[0x9165] = "俺は貴様を倒す！この俺のプライドを・・",
	[0x9166] = "俺の全てをお前はふみにじった！",
	[0x9167] = "その報いを受けてもらう！",

	-- Stage 8 / Scene 2 (プライドと守るもの)
	[0x9261] = "貴様さえいなければ・・貴様の存在さえ抹消すれば俺は・・・・",
	[0x9211] = "俺は死ぬ訳にはいかない。俺には俺を待っている人が・・・バーシスのクルーが・・ ・・・そしてクレアがいるんだ！",

	-- Stage 8 / Scene 3 (ベルダーク少佐撃破・終局)
	[0x9361] = "なぜだ・・・なぜこの俺が・・・・ ぐわぁ！！",

	}

	local VOICE_TYPE_MAP = {
		[0x858835] = 1,
		[0x85889C] = 1,
		[0x858ADF] = 1,
		[0x858B5A] = 1,
	...
	local voicevox_ip = "127.0.0.1"
	local voicevox_port = 12345
	local voicevox_output = nil
	function voicevox_tcp_connected()
		if voicevox_output then return true end
		local socket_path = string.format("socket.%s:%d", voicevox_ip, voicevox_port)
		...
		print("[Error] internal socket failed to open. Check if App is running and firewall allows port " .. voicevox_port)
		return false
	end
	voicevox_tcp_connected()
	function voicevox_send_command(text, style_id, speedScale, pitchScale, intonationScale, volumeScale)
		if not voicevox_tcp_connected() then return end

		-- Format: voicevox:style_id:speed:pitch:intonation:volume:text
		local message = string.format("voicevox:%d:%.2f:%.2f:%.2f:%.2f:%s\n", 
			style_id, speedScale, pitchScale, intonationScale, volumeScale, text)

		local success, err = pcall(function()
			voicevox_output:write(message)
		end)

		if not success then
			print("[Error] VOICEVOX transmission failed: " .. tostring(err))
			if tcp_output then
				pcall(function() voicevox_output:close() end)
				voicevox_output = nil
			end
		end
	end
	...
	local text_addr = 0
	local text_count = 0
	function valken_chara_text2(offset)
		local pc_reg = cpu.state["PC"].value
		local s_reg = cpu.state["S"].value
		local a_reg = cpu.state["A"].value
		local x_reg = cpu.state["X"].value
		local y_reg = cpu.state["Y"].value
		local p_reg = cpu.state["D"].value
		local val = mem:read_u16(s_reg)
		MES("A:%02X", a_reg)
		if val ~= v_buf and a_reg16 > 0xff then
			if a_reg16 ~= a_reg16_buf then
				MES("PC:%06X S:%04X A:%04X X:%04X Y:%04X P:%04X V:%04X", pc_reg, s_reg, a_reg16, x_reg, y_reg, p_reg, val)
			end
			a_reg16_buf = a_reg16
		end
		v_buf = val
		if v_count == 0 then a_reg16 = a_reg end
		if v_count == 1 then a_reg16 = a_reg16 + (a_reg << 8) v_buf = 0 end
		v_count = v_count + 1
		if a_reg == 0x00FC or a_reg == 0x00FA or a_reg == 0x00FD then v_buf = 0 v_count = 0 end
	end
	...
	local a_buf = 0
	function valken_chara_text_fmod(offset)
		local a_reg = cpu.state["A"].value
		--MES("A:%06X I:%04X", text_addr, a_reg)
		if text_addr ~= v_buf then
			local voice_id = DialogueManager.get_id(text_addr, a_reg)
			if voice_id > 0x1000 then
				fmod:play(voice_id)
			else
				MES("* A:%06X I:%04X", text_addr, a_reg)
			end
		...
		if text_addr ~= v_buf then
			...
			if voice_id and voice_id > 0x1000 then
				if voice_play(voice_id,text_addr) == 0 then
					local char_id = (voice_id >> 4) & 0x0F
					local text = TEXT_MAP[voice_id]
					local v_param = CHAR_VOICE_MAP[char_id]

					if text and v_param then
						-- Send to C++ Server: text, style_id, speed, pitch, intonation, volume
						--voicevox_send_command(text, v_param[1], v_param[2], v_param[3], v_param[4], v_param[5])
					else
						print(string.format("[VOICEVOX] Text or Character parameter missing for ID: 0x%04X", voice_id))
					end
				end
			end
	...
	function voice_play(voice,text_addr)
		if fmod:is_samples(voice) > 0 then
			local voice_type = VOICE_TYPE_MAP[text_addr] or 0
